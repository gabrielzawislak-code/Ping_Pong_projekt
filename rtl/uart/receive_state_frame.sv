        ...
        case(state)
            ...
            TERM: begin
                if(!rx_empty) begin
                    ...
                    // TEMPORARY DIAGNOSTIC: hamming_decode() taken
                    // completely out of the picture - only the
                    // terminator is verified, and fields are unpacked
                    // straight from raw_payload (uncorrected), same as
                    // before Hamming was added. This isolates whether
                    // hamming_decode() itself is why the CLIENT never
                    // accepts a state frame. Revert once confirmed either
                    // way (put back the decoded[...]/status version).
