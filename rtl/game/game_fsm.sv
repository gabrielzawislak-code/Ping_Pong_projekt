 ...
 *
 * Description:
 * Local game state machine, identical on both boards (HOST and CLIENT
 * run this same module). flag_char encodes what should be shown/done on
 * screen and is also the value broadcast to the other board over UART
 * (as part of its outgoing frame) so the peer can tell whether we are
 * idle, ready, playing or finished - peer_state is that same field
 * coming back from the other board:
 *   3'b001 - IDLE    : "press button C to start" screen
 ...
 *
 * The PLAYING -> END transition compares against score_1/score_2, which
 * for the HOST are its own, locally computed score, and for the CLIENT
 * are the score fields mirrored from the HOST's frame - so both boards
 * reach END at (within one UART round trip of) the same instant.
 */
module game_fsm(
    input logic clk,
    input logic rst_n,
